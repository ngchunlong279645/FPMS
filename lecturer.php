...
if (isset($_GET['submit'])) {
    ...
    if ($operation == 'delete') {
        ...
        $sqldeletelecinfo = "DELETE FROM `tbl_lecturer` WHERE user_id = '$user_id'";
        $conn->exec($sqldeletelecinfo);
        echo "<script>alert('Info deleted')</script>";
        echo "<script>window.location.replace('lecturer.php')</script>";
    }
} else {
    $sqllecturer = "SELECT * FROM tbl_lecturer";
}
...
<a href="index.php" class="button w3-right logout-button">Logout</a>
  ...
  <img src="../user/lecturer/<?php echo $user_id?>.png" onerror="this.onerror=null;this.src='pic/lecturer.webp'" >
  </div>
