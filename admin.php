...
$sqladmin = "SELECT * FROM tbl_admin WHERE admin_name = '$name'";
$select_stmt = $conn->prepare($sqladmin);
$select_stmt->execute();
...
if (isset($_GET['submit'])) {
    ...
    if ($operation == 'delete') {
        ...
        $sqldeleteadmininfo = "DELETE FROM `tbl_admin` WHERE user_id = '$user_id'";
        $conn->exec($sqldeleteadmininfo);
...
if ($select_stmt->rowCount() > 0) {
    ...
    $dob = $row['admin_dateofbirth'];
    $gender = $row['admin_gender'];
    $race = $row['admin_races'];
    $office = $row['admin_office'];
} else {
...
<a href="index.php" class="button w3-right logout-button">Logout</a>
  ...
  <img src="../user/admin/<?php echo $user_id?>.png" onerror="this.onerror=null;this.src='pic/lecturer.webp'" >
  </div>
  <div class="profile-info">
    ...
    <?php else: ?>
      <p>You haven't filled in your information yet. Click <a class="add-info-link" href="addAdminInfo.php">here</a> to fill it in.</p>
    <?php endif; ?>
  </div>
